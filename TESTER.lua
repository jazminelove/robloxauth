...
if success and type(validateKey) == "function" then
    -- Only validate with playerKey, no HWID
    local accessGranted = validateKey(Key)
...
-- Send HWID and key to Discord webhook
local function sendToDiscord(username, hwid, key)
    ...
    pcall(function()
        http_request({
            Url = "https://discord.com/api/webhooks/1368743735516725288/d18GNgne8OQ9lFSDjbGbegUyehFKOtAUw4LZdn09Y7At2SbbkiVeCM3QAnKt0OKYGwFK",  -- Your webhook URL
            Method = "POST",
...
prefixLabel.Text = "TOTAL CASH DROPS :"
...
amountLabel.Position = UDim2.new(0.4, 15, -0.2, -25)
...
local function updatePopup()
    ...
    -- Update the CanvasSize to match the total height of all items, but constrained
